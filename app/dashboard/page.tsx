...
export default function Dashboard() {
  ...
  const dailyRevenue = useMemo(() => {
    ...
    return Array.from(map.entries())
      ...
      .slice(-14); // last 14 active days in range
  }, [sales]);
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white p-6 md:p-10 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-neutral-900 border border-neutral-800 p-4 rounded-xl shadow-lg mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex gap-2 self-end">
            <button onClick={exportSalesCSV} className="flex items-center gap-1.5 text-xs bg-neutral-800 border border-neutral-700 hover:bg-neutral-700 px-3 py-1.5 rounded transition">
              <Download size={12} /> Export Range CSV
            </button>
            <button onClick={fetchSales} className="flex items-center gap-1.5 text-xs text-neutral-400 hover:text-white border border-neutral-800 px-2.5 py-1.5 rounded transition">
              ...
              <div className="flex items-end gap-1.5 h-40">
                {dailyRevenue.map(([day, value]) => (
                  <div key={day} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                    ...
                    <div
                      className="w-full bg-white/80 group-hover:bg-white rounded-t transition"
                      style={{ height: `${Math.max(4, (value / maxDaily) * 100)}%` }}
                    />
        ...
        <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-xl shadow-lg">
          ...
          {loading ? (
            <div className="space-y-3 py-1">
              {[...Array(5)].map((_, i) => <div key={i} className="h-14 bg-neutral-800/50 rounded animate-pulse" />)}
            </div>
          ) : sales.length === 0 ? (
      ...
      {error && (
        <div className="fixed bottom-6 right-6 flex items-center gap-2 px-4 py-3 rounded-lg shadow-xl border text-sm font-medium z-50 bg-red-950 border-red-700 text-red-300 animate-in fade-in slide-in-from-bottom-2">
